...
for REYNOLDS_NUMBER in Rnumbers:
    ...
    V_MAT = np.zeros((N_ITERATIONS//PLOT_EVERY_N_STEPS,N_POINTS_X,N_POINTS_Y))
    ...
    if True:
        ...
        for iteration_index in tqdm(range(N_ITERATIONS)):
            ...
            if iteration_index % PLOT_EVERY_N_STEPS == 0:
                ...
                V_MAT[count] = velocity_magnitude
                count += 1
    ...
    with open("flow_full{}.p".format(REYNOLDS_NUMBER), "wb") as f:
            pickle.dump([V_MAT,CYLINDER_CENTER_INDEX_X, CYLINDER_CENTER_INDEX_Y,
            CYLINDER_RADIUS_INDICES,PLOT_EVERY_N_STEPS], f)